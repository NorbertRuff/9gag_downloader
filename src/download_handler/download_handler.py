...
class DownloadHandler:
    ...
    def try_video_download(self, gag_id: str, gag_title: str) -> bool:
        ...
        if os.path.exists(f"{self.destination_folder}/{self.VIDEO_SAVE_LOCATION}/{sanitized_title}.mp4"):
        ...
        if response.status_code == 200:
            ...
            with open(f"{self.destination_folder}/{self.VIDEO_SAVE_LOCATION}/{sanitized_title}.mp4", "wb") as f:
    ...
    def try_image_download(self, gag_id: str, gag_title: str) -> bool:
        ...
        sanitized_title = re.sub(r'[\\/*?:"<>|]', "", gag_title)
        if os.path.exists(f"{self.destination_folder}/{self.IMAGE_SAVE_LOCATION}/{sanitized_title}.jpg"):
        ...
        if response.status_code == 200:
            ...
            with open(f"{self.destination_folder}/{self.IMAGE_SAVE_LOCATION}/{sanitized_title}.jpg", "wb") as f:
                f.write(response.content)
            return True
        return False
